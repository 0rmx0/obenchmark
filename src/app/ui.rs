...
use iced::time;
use iced::widget::{button, column, horizontal_rule, progress_bar, row, scrollable, text};
        ...
        match msg {
            ...
            Msg::Export => {
                if let AppState::Showing(result) = &self.state {
                    ...
                    let _ =
                        std::fs::write(format!("bench_{}.json", Local::now().timestamp()), json);
                }
            }
            ...
            AppState::Showing(result) => {
                ...
                if let Some(si) = &result.system_info {
                    rows = rows
                        .push(text(format!(
                            "CPU Vendor: {}",
                            si.cpu.vendor.clone().unwrap_or("unknown".to_string())
                        )))
                        .push(text(format!(
                            "CPU Model: {}",
                            si.cpu.model.clone().unwrap_or("unknown".to_string())
                        )))
